...
final class SingleDisplayGammaTechnique {
    ...
    private let hdrPollIntervalMs: UInt64 = 500
    ...
    private func gammaFactorCompensatedForEDR(userBrightness: Float, screen: NSScreen) -> Float {
        ...
        let effective = 1.0 + (Double(userBrightness) - 1.0) * min(maxEdr, 4.0) / 4.0
        return Float(min(effective, Double(screenHelperDeviceMaxBrightness())))
    }
